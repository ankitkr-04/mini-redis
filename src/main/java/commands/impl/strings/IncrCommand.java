...
public final class IncrCommand extends WriteCommand {
    ...
    private static final String COMMAND_NAME = "INCR";
    ...
    protected CommandResult executeInternal(CommandContext context) {
        ...
        try {
            long incrementedValue = context.getStorageService().incrementString(key);
            publishDataAdded(key, context.getServerContext());
            propagateCommand(context.getArgs(), context.getServerContext());

            LOGGER.debug("Key '{}' incremented, new value: {}", key, incrementedValue);
            return CommandResult.success(ResponseBuilder.integer(incrementedValue));
        } catch (IllegalStateException | NumberFormatException exception) {
            LOGGER.info("Failed to increment key '{}': {}", key, exception.getMessage());
