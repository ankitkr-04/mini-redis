...
public final class MetricsHandler {
    ...
    public String getMetricsAsInfo() {
        ...
        result.append("uptime_seconds:").append(String.format("%.0f", (Double) allMetrics.get("uptime_seconds"))).append("\r\n");
        result.append("\r\n");
        ...
        if (storageMetrics != null && !storageMetrics.isEmpty()) {
            ...
            storageMetrics.forEach((key, value) -> 
                result.append(key).append(":").append(value).append("\r\n"));
            result.append("\r\n");
        }
        ...
        if (replicationMetrics != null && !replicationMetrics.isEmpty()) {
            ...
            replicationMetrics.forEach((key, value) -> 
                result.append(key).append(":").append(value).append("\r\n"));
            result.append("\r\n");
        }
        ...
        if (pubsubMetrics != null && !pubsubMetrics.isEmpty()) {
            ...
            if (subscriberCounts != null && !subscriberCounts.isEmpty()) {
                subscriberCounts.forEach((channel, count) -> 
                    result.append("subscribers_").append(channel).append(":").append(count).append("\r\n"));
            }
            result.append("\r\n");
        }
        ...
        if (transactionMetrics != null && !transactionMetrics.isEmpty()) {
            ...
            transactionMetrics.forEach((key, value) -> 
                result.append(key).append(":").append(value).append("\r\n"));
            result.append("\r\n");
        }
        ...
        if (persistenceMetrics != null && !persistenceMetrics.isEmpty()) {
            ...
            persistenceMetrics.forEach((key, value) -> 
                result.append(key).append(":").append(value).append("\r\n"));
            result.append("\r\n");
        }
     ...
     * 
     * @param section Section name (server, commands, storage, replication, pubsub, transactions, persistence)
    ...
    public String getMetricsAsPrometheus() {
        ...
        appendPrometheusMetric(result, "redis_connected_clients", allMetrics.get("active_connections"), "Number of client connections");
        appendPrometheusMetric(result, "redis_commands_processed_total", allMetrics.get("total_commands_processed"), "Total number of commands processed");
        appendPrometheusMetric(result, "redis_errors_total", allMetrics.get("total_errors"), "Total number of errors");
        appendPrometheusMetric(result, "redis_memory_usage_bytes", allMetrics.get("memory_usage_bytes"), "Memory usage in bytes");
        appendPrometheusMetric(result, "redis_uptime_seconds", allMetrics.get("uptime_seconds"), "Server uptime in seconds");
    ...
    private void appendPrometheusMetricWithTags(StringBuilder sb, String name, Object value, String help, String... tags) {
        sb.append("# HELP ").append(name).append(" ").append(help).append("\n");
